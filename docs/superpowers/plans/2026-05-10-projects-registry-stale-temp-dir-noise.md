...
- [ ] **Step 2: Build the CLI bundle (required by `pipeline-preflight.test.ts`'s `spawnSync`)**
...
- [ ] **Step 2: For each candidate, grep for `pipelineRunCommand` callers that pass `project`**
...
- [ ] **Step 3: Re-run the empirical check after each fix**

After each candidate is patched, redo Task 4.1 Steps 1–5. Stop when the grep returns `0`.

- [ ] **Step 4: Commit each follow-up fix as its own commit**
...
- [ ] **Step 1: Confirm clean registry on a fresh run**
...
- [ ] **Step 2: No commit needed if Task 4.2 was skipped.**

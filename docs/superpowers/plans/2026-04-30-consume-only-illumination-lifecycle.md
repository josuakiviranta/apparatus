...
- [ ] **Step 1: Add `consume` to the named imports**
...
- [ ] **Step 2: Write failing tests for `consume`**
...
- [ ] **Step 3: Run tests — confirm they fail at the import boundary**
...
- [ ] **Step 4: Implement `consume()` in `illumination-server.ts`**
...
The janitor pipeline graph (`src/cli/pipelines/janitor/pipeline.dot`) is unchanged — still single-agent (`start → read_vision → janitor → done`). Only the agent's prompt and the test asserting its contract change.

**Files:**
- Rewrite: `src/cli/pipelines/janitor/janitor.md` (entire body + frontmatter `tools:` list)
...
**Files:**
- Rewrite: `src/cli/pipelines/janitor/janitor.md` (entire content)

- [ ] **Step 1: Replace the file contents**

Overwrite `src/cli/pipelines/janitor/janitor.md` with:
...
Run: `npx ralph pipeline validate src/cli/pipelines/janitor/pipeline.dot`
Expected: PASS — agent file resolves, frontmatter validates.

- [ ] **Step 4: Commit**

```bash
git add src/cli/pipelines/janitor/janitor.md
...
```bash
ralph heartbeat pipeline src/cli/pipelines/janitor/pipeline.dot --project . --every 720
```
...
(Note the path correction: `pipelines/janitor.dot` → `src/cli/pipelines/janitor/pipeline.dot` — per-folder layout.)
...
```bash
grep -nE "consume|mark_implemented|mark_plan_implemented|list_plans|dispatched" src/cli/pipelines/janitor/janitor.md
grep -nE "kiss|bloat|yagni|refactor" -i src/cli/pipelines/janitor/janitor.md
```
...
- [ ] Run `ralph heartbeat pipeline src/cli/pipelines/janitor/pipeline.dot --project <test-project> --every 720` once (single iteration) and confirm the janitor either writes a `janitor-<area>` illumination or exits cleanly with no candidate.

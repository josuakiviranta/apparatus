...
## Chunk 1: Runtime guard — `expandVariables` throws on undefined variables

### File Map

| Action | File | Purpose |
|--------|------|---------|
| Modify | `src/attractor/transforms/variable-expansion.ts` | Add `UndefinedVariableError`, throw on undefined vars, support defaults |
| Modify | `src/attractor/tests/transforms.test.ts` | Fix broken assertion, add throw + default tests |

---

### Task 1: Update transform tests (red)

**Files:**
- Modify: `src/attractor/tests/transforms.test.ts`

- [ ] **Step 1: Read the current test file**

```bash
cat -n src/attractor/tests/transforms.test.ts
```

- [ ] **Step 2: Update "leaves unknown variables as-is" test to expect a throw**

Find the test that asserts `"Value: $unknown.key"` is returned unchanged. Replace it with a test that expects `UndefinedVariableError` to be thrown:
...
- [ ] **Step 3: Add test for default value substitution**
...
- [ ] **Step 4: Add test confirming defined variables still expand normally**
...
```bash
npx vitest run src/attractor/tests/transforms.test.ts
```

Expected: FAIL — `UndefinedVariableError` not exported, `expandVariables` does not accept third arg

---

### Task 2: Implement UndefinedVariableError + expand defaults (green)

**Files:**
- Modify: `src/attractor/transforms/variable-expansion.ts`

- [ ] **Step 6: Read the current source file**

```bash
cat -n src/attractor/transforms/variable-expansion.ts
```

- [ ] **Step 7: Add UndefinedVariableError class**

Add before `expandVariables`:
...
- [ ] **Step 8: Update expandVariables signature and undefined handling**

Change the function signature to accept an optional `defaults` parameter:
...
Replace the undefined passthrough (line 12: `if (v === undefined) return match;`) with:
...
- [ ] **Step 10: Update callers to pass defaults where needed**

Search for all `expandVariables` call sites:

```bash
grep -rn "expandVariables" src/ --include="*.ts" | grep -v test | grep -v ".d.ts"
```

For each caller that currently relies on the silent passthrough behavior, either:
- Pass a `defaults` map for intentionally optional variables, OR
- Ensure the variable is always defined in context before expansion

Key callers to check:
- `src/attractor/handlers/tool.ts` — `expandVariables(node.toolCommand, ctx.values)`
- Any other handler that calls `expandVariables`
...
Expected: All tests PASS — no callers relied on silent passthrough for variables that are actually always defined

- [ ] **Step 12: Commit**

```bash
git add src/attractor/transforms/variable-expansion.ts src/attractor/tests/transforms.test.ts
git commit -m "fix(pipeline): expandVariables throws UndefinedVariableError instead of silent passthrough"
```

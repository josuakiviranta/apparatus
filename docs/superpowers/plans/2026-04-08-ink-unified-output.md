...
- [ ] **Step 2: Update new.ts** — same pattern.

- [ ] **Step 3: Update meditate-create.ts** — same pattern.
...
**Precondition:** All `@clack/prompts` imports must be gone.

- [ ] **Step 1: Verify no Clack imports remain**

```bash
grep -r "@clack/prompts" src/
```

Expected: no output (or only test mock files that reference the old mock, which can be cleaned up).

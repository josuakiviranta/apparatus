...
- [ ] **Step 1: Verify no untracked files in `specs/`**
...
- [ ] **Step 2: `git mv` the folder**

```bash
git mv specs docs/specs
```

- [ ] **Step 3: Fix internal cross-links inside `docs/specs/*.md`**
...
- [ ] **Step 4: Commit (move only, no other edits yet — leaves bisect clean)**
...
- [ ] **Step 2: Edit nav links (lines ~176–178)**
...
- [ ] **Step 3: Edit example var (line ~53)**

If the line reads `--var specs_dir=docs/specs`, leave as-is — it already matches the new location. Only act if it reads `specs/`.

- [ ] **Step 4: Rewrite the "specs/ vs docs/superpowers/specs/" distinction (Directory Map row at lines ~156–157, plus the callout block at line ~163)**
...
- [ ] **Step 1: Read inventory entries that mention `specs/`**
...
- [ ] **Step 1: Dispatch plan-document-reviewer subagent**
...
- [ ] **Step 2: Apply fixes if ❌, re-dispatch until ✅**
...
**Files:**
- Modify: `src/cli/pipelines/janitor/janitor.md:56` — `specs/*.md` → `$specs_dir/*.md` (the only filesystem path-ref in janitor — illuminations and plans are MCP-resolved, not filesystem paths)
- Modify: `src/cli/pipelines/janitor/pipeline.dot:4` — current `inputs="project"` → `inputs="project, specs_dir"`
...
```bash
node dist/cli/index.js pipeline validate src/cli/pipelines/janitor/pipeline.dot
```

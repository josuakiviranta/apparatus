...
2. **Walk the project surface.** Use `project_tree` to orient. Then `glob_files` and `Grep` to scan source for KISS-lens candidates:
   ...
   - **Deepening opportunities:** modules whose interface is large relative to what they hide; a concept implemented twice (e.g. front end + back end, or two layers) with no single seam forcing them to agree; wrappers that expose internals instead of concealing them; "manager" / "helper" surfaces with little implementation behind them. Suggest collapsing toward a deeper module — smaller interface, more implementation behind it — to raise locality (changes concentrate in one place) and leverage (caller learns less, gets more capability).

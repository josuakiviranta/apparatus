...
describe(".apparat/scenarios/store/ — chunk-4 per-folder migration", () => {
  it("pipeline.dot exists at <repo>/.apparat/scenarios/store/pipeline.dot", () => {
    const expected = join(REPO_ROOT, ".apparat", "scenarios", "store", "pipeline.dot");
    expect(existsSync(expected)).toBe(true);
  });

  it("validateGraph emits zero error-level diagnostics for the migrated pipeline", () => {
    const dotPath = join(REPO_ROOT, ".apparat", "scenarios", "store", "pipeline.dot");

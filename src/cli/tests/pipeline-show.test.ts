...
describe("pipelineShowCommand", () => {
  ...
  it("resolves a bare name through pipeline-resolver and writes SVG into <project>/.apparat/pipelines/", async () => {
    const { mkdirSync, writeFileSync } = await import("fs");
    mkdirSync(join(dir, ".apparat", "pipelines"), { recursive: true });
    writeFileSync(
      join(dir, ".apparat", "pipelines", "review.dot"),
      `digraph g {\n  start [shape=Mdiamond]\n  done [shape=Msquare]\n  start -> done\n}`,
    );
    const code = await pipelineShowCommand("review", { project: dir });
    expect(code).toBe(0);
    expect(existsSync(join(dir, ".apparat", "pipelines", "review.svg"))).toBe(true);
  });
});

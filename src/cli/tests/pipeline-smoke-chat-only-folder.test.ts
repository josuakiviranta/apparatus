...
describe("pipelines/smoke/chat-only/ — chunk-4 per-folder migration", () => {
  it("pipeline.dot exists at <repo>/pipelines/smoke/chat-only/pipeline.dot", () => {
    const expected = join(REPO_ROOT, "pipelines", "smoke", "chat-only", "pipeline.dot");
    expect(existsSync(expected)).toBe(true);
  });

  it("ships chat.md alongside pipeline.dot for project-local agent resolution", () => {
    const agentPath = join(REPO_ROOT, "pipelines", "smoke", "chat-only", "chat.md");
  ...
  it("validateGraph emits zero error-level diagnostics for the migrated pipeline", () => {
    const dotPath = join(REPO_ROOT, "pipelines", "smoke", "chat-only", "pipeline.dot");

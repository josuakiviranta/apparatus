...
describe(".apparat/scenarios/static-multi-node/ — chunk-4 per-folder migration", () => {
  it("pipeline.dot exists at <repo>/.apparat/scenarios/static-multi-node/pipeline.dot", () => {
    const expected = join(REPO_ROOT, ".apparat", "scenarios", "static-multi-node", "pipeline.dot");
    expect(existsSync(expected)).toBe(true);
  });

  it("ships task.md alongside pipeline.dot for project-local agent resolution", () => {
    const agentPath = join(REPO_ROOT, ".apparat", "scenarios", "static-multi-node", "task.md");
  ...
  it("validateGraph emits zero error-level diagnostics for the migrated pipeline", () => {
    const dotPath = join(REPO_ROOT, ".apparat", "scenarios", "static-multi-node", "pipeline.dot");

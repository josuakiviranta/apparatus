...
describe("resolvePipelineArg", () => {
  ...
  it("resolves a plain name to project-local path when it exists", () => {
    mockExists.mockImplementation((p: unknown) =>
      typeof p === "string" && p.includes(join("/my-app", ".apparat", "pipelines", "review.dot"))
    );
    const result = resolvePipelineArg("review", "/my-app");
    expect(result).toBe(join("/my-app", ".apparat", "pipelines", "review.dot"));
  });
...
describe("resolvePipelineArg bundled fallback", () => {
  ...
  it("prefers project-local pipeline when it exists", () => {
    mockExists.mockImplementation((p: unknown) =>
      typeof p === "string" && p.includes("/my/project/.apparat/pipelines/implement.dot")
    );
    const result = resolvePipelineArg("implement", "/my/project");
    expect(result).toContain("/my/project/.apparat/pipelines/implement.dot");
  });
...
describe("resolvePipelineArg folder-form lookup (Chunk 4: per-pipeline folder)", () => {
  ...
  it("returns <project>/.apparat/pipelines/<name>/pipeline.dot when folder-form exists and flat-form does not", () => {
    const folderPath = join("/my-app", ".apparat", "pipelines", "janitor", "pipeline.dot");
    mockExists.mockImplementation((p: unknown) =>
  ...
  it("prefers folder-form over flat-form when BOTH exist (folder = SSoT, Decision 1)", () => {
    const folderPath = join("/my-app", ".apparat", "pipelines", "janitor", "pipeline.dot");
    const flatPath = join("/my-app", ".apparat", "pipelines", "janitor.dot");
    mockExists.mockImplementation((p: unknown) =>
  ...
  it("falls through to flat-form when only the flat <name>.dot exists (back-compat)", () => {
    const flatPath = join("/my-app", ".apparat", "pipelines", "legacy.dot");
    mockExists.mockImplementation((p: unknown) =>
...
describe("getPipelinesDir", () => {
  it("returns .apparat/pipelines subfolder of project", () => {
    expect(getPipelinesDir("/my-app")).toBe(join("/my-app", ".apparat", "pipelines"));
  });
});

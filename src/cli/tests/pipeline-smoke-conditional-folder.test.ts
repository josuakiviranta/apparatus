...
describe("pipelines/smoke/conditional/ — chunk-4 per-folder migration", () => {
  it("pipeline.dot exists at <repo>/pipelines/smoke/conditional/pipeline.dot", () => {
    const expected = join(REPO_ROOT, "pipelines", "smoke", "conditional", "pipeline.dot");
    expect(existsSync(expected)).toBe(true);
  });

  it("ships task.md alongside pipeline.dot for project-local agent resolution", () => {
    const agentPath = join(REPO_ROOT, "pipelines", "smoke", "conditional", "task.md");
  ...
  it("validateGraph emits zero error-level diagnostics for the migrated pipeline", () => {
    const dotPath = join(REPO_ROOT, "pipelines", "smoke", "conditional", "pipeline.dot");

...
describe(".apparat/scenarios/tool/ — chunk-4 per-folder migration", () => {
  it("pipeline.dot exists at <repo>/.apparat/scenarios/tool/pipeline.dot", () => {
    const expected = join(REPO_ROOT, ".apparat", "scenarios", "tool", "pipeline.dot");
    expect(existsSync(expected)).toBe(true);
  });

  it("validateGraph emits zero error-level diagnostics for the migrated pipeline", () => {
    const dotPath = join(REPO_ROOT, ".apparat", "scenarios", "tool", "pipeline.dot");

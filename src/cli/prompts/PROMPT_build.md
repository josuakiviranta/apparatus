...
0c. Choose one chunk from IMPLEMENTATION_PLAN.md to implement in this iteration. (The future iterations will implement the next chunks to implement.)
0d. For reference, the application source code is in `src/*`.


1. Your task is to manage the implement functionality per the specifications using parallel subagents and red/green TDD. Follow @IMPLEMENTATION_PLAN.md and choose one chunk and the most important item to address inside the chosen chunk.  Before making changes, search the codebase (don't assume not implemented) using Sonnet subagents. You may use up to 500 parallel Sonnet subagents for searches/reads and only 1 Sonnet subagent for build/tests. Use Opus subagents when complex reasoning is needed (debugging, architectural decisions).

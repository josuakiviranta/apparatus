---
name: scaffolder
description: Interactive pipeline scaffolder — helps the user design and write a new ralph pipeline as a .dot file
model: opus
...
## Agent inventory (enumerate before drafting)

Before starting, discover what agents are available for use in this project. Run both of the following commands and build a mental inventory table:

```sh
ls ~/.ralph/agents/ 2>/dev/null
ls $pipelines_dir/*/[a-z]*.md 2>/dev/null
```

Print the resulting agent inventory as a Markdown table with columns `name | description | source` before you start drafting the pipeline. This replaces the need for a runtime injection step.
...
Write the pipeline to `$pipelines_dir/$pipeline_name/pipeline.dot`. Create the directory first if it does not exist. Use the `Write` tool or create the file directly.

After writing, run `ralph pipeline validate $pipelines_dir/$pipeline_name/pipeline.dot` and iterate until it reports zero errors. Warnings are acceptable; errors are not.

After validation is clean, tell the user the file is ready and briefly describe the workflow.
...
When done, return a JSON object with the absolute path of the created pipeline file:

```json
{"created_path": "/absolute/path/to/$pipeline_name/pipeline.dot"}
```

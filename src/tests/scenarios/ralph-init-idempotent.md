...
- Overwrite the scaffolded files with sentinel content:
  - write `CUSTOM VISION` to `idem-smoke/.ralph/VISION.md`
  - write `CUSTOM CONTEXT` to `idem-smoke/.ralph/CONTEXT.md`
...
## Expect
- exit code is 0
- file `idem-smoke/.ralph/VISION.md` content is exactly `CUSTOM VISION` (not overwritten)
- file `idem-smoke/.ralph/CONTEXT.md` content is exactly `CUSTOM CONTEXT` (not overwritten)

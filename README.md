...
**Manual sweep.** `apparat sweep [project-folder]` lists every `.apparat/`
substrate with size and a curated/scratch tag. Scratch entries (`runs/<runId>/`,
both `.triage/<runId>/`, `sessions/`) are pre-selected; curated entries
(`illuminations/*.md`, `stimuli/*.md`, `pipelines/`, `scenarios/`, `notes.md`,
`lessons/`, `reasoning-memory/`) prompt for confirmation before delete.
Pass `--dry-run` to see the list without prompting. In a non-TTY shell the
command prints the list, writes a hint to stderr, and exits 0 — mirroring
`apparat pipeline show`'s `--open` policy. As of 2026-05-13, pipelines no
longer write `.apparat/sessions/` files at the tail — any sediment under
`.apparat/sessions/` is pre-rule and reapable via `apparat sweep`.
...
- `apparat status <projectPath> <pipelineName> <runId>` — zoom into one run: trace renderer. Auto-tails live if the run is in-progress; static replay if finished. Strips the same ceremony as `apparat pipeline trace` by default; pass `--full` for the raw stream. Each closed block carries a dim per-node delta line — the same `+ key=value` markers the CLI roster shows. `--full` disables that synthesis and emits the raw JSONL stream end-to-end.

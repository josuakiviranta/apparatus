...
- [x] `ralph pipeline run pipelines/illumination-to-plan.dot` (if you have time for a dry run) completes the same way it did before.
...
- [x] **Step 2: Write a regression test asserting that numeric/boolean/object values survive a round trip through `Outcome`**
...
- [x] **Step 3: Run the test and verify it fails with type errors**
...
- [x] **Step 4: Widen `Outcome.contextUpdates`, `CheckpointState.context`, and `PipelineContext.values`**
...
- [x] **Step 5: Run the test and verify it passes (type errors gone)**
...
- [x] **Step 1: Identify every place engine.ts types context as `Record<string, string>`**
...
- [x] **Step 2: Write a test that puts a numeric contextUpdate through a full engine run**
...
- [x] **Step 4: Widen the engine's context typing**
...
- [x] **Step 6: Run the full test suite to catch ripples**
...
- [x] **Step 1: Confirm the existing test file covers both transforms**
...
- [x] **Step 2: Append a failing test block for non-string coercion**
...
- [x] **Step 1: Append a failing test block to transforms.test.ts**
...
- [x] **Step 3: Widen the signature and implement coercion**
...
- [x] **Step 1: Write a failing test for non-string LHS comparison**
...
- [x] **Step 2: Run, verify failure (type error or comparison mismatch)**
...
- [x] **Step 3: Widen `ContextMap` and coerce in `resolveKey`**
...
- [x] **Step 1: Write a failing test for round-trip of non-string values**
...
- [x] **Step 2: Run, verify it either already passes OR fails on a type check**
...
- [x] **Step 3: If the test passes, no code change needed — commit the test**
...
- [x] **Step 3: Sanity-check existing pipeline parses and executes a dry step**
...
- [x] **Step 4: Mark Chunk 1 complete**
...
- [x] `npm test` green.
- [x] New tests for B.1 and B.2 added and passing.
- [x] No regressions in `graph.test.ts` on unquoted identifier values.
...
- [x] **Step 1: Write a failing test for multi-line prompt via `\n`**
...
- [x] **Step 3: Implement scoped unescape**
...
- [x] **Step 4: Run, verify all tests pass including unquoted regression**

Run: `npx vitest run src/attractor/tests/graph.test.ts`
Expected: all tests (new + existing) pass.

- [x] **Step 5: Run full suite to catch collateral**
...
- [x] **Step 3: Sanity-check `illumination-to-plan.dot` still parses**
...
**Verification after chunk:**
- [x] New test files all pass.
- [x] `npm run build` succeeds.
- [x] No behavioral change to any existing command.
...
- [x] **Step 2: Run the test and verify it fails (module does not exist yet)**

Run: `npx vitest run src/cli/tests/session.test.ts`
Expected: FAIL — `Cannot find module '../lib/session.js'`.

- [x] **Step 3: Create `src/cli/lib/session.ts`**
...
- [x] **Step 3: Create `src/cli/lib/slash-commands.ts`**
...
- [x] **Step 3: Create `src/cli/lib/stream-json-input.ts`**
...
**Verification after chunk:**
- [x] New `runInteractive()` contract tests pass against a fake child process (no real Claude spawn).
- [x] `agent.run()` behavior is unchanged — all existing `agent.test.ts` tests still pass.
...
- [x] **Step 3: Append `parseStreamJsonEvents` and `StreamJsonEvent` union to `src/cli/lib/stream-formatter.ts`**
...
- [x] **Step 5: Verify existing `streamEvents()` is untouched by running its tests**
...
- [x] **Step 3: Implement `RunInteractiveOptions`, `ChildHandle`, `buildInteractiveArgs`, and `runInteractive`**
...
- [x] **Step 4: Run the contract tests, verify pass**

Run: `npx vitest run src/cli/tests/agent-interactive.test.ts`
Expected: all tests pass.

- [x] **Step 5: Run the existing `agent.test.ts` to verify `run()` is untouched**

...
> **Status:** Implemented on 2026-04-18. Chunks 1+2+3 complete. Commits: cd210d7 (tracer), aee7f7f (CLI stderr + integration test).
...
- [x] **Step 3: Implement — extend `onNodeEnd` in the tracer**
...
- [x] **Step 1: (Pre-req) Honour `RALPH_RUNS_ROOT` env var for trace dir**
...
- [x] **Step 2: Capture last failing node inside the existing `onNodeEnd` callback**
...
- [x] **Step 3: Print the one-line summary in `finally`**
...
- [x] **Step 2: Trigger a known failure**
...
- [x] **Step 5: Done — no commit needed for this verification chunk**
...
- [x] Tracer unit tests (Chunk 1) green.
- [x] Integration + stderr test (Chunk 2) green.
- [x] `npm run test` clean.
- [x] Manual smoke (Chunk 3) shows both stderr summary and trace-enriched failureReason.
- [x] Two commits on `main` (or the feature branch): one per chunk. (Chunk 3 is verification-only.)

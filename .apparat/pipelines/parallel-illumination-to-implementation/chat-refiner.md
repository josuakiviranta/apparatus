...
5. **Before ending the session**, write your **agreed conclusions** to `$project/.apparat/meditations/illuminations/.triage/$run_id/chat-notes.md` — this is the **exact path the downstream `chat-summarizer` reads**. Writing anywhere else (or inventing a path) silently drops every conclusion, because the summarizer reads only that location. Your Write tool creates the `.triage/$run_id/` parent directory automatically. Format the file so the downstream summarizer can attribute every conclusion back to *what the user said and why*:

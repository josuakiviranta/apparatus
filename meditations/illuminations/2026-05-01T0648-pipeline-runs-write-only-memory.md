---
date: 2026-05-01
description: Pipeline runs persist rich JSONL state under ~/.ralph/<projectKey>/runs/<runId>/ but the CLI exposes no `runs` list — `pipeline trace` requires a runId you can only discover by triggering --resume errors or shelling into the directory, making the per-run memory effectively write-only.
---

## Core Idea

Every `pipeline run` writes a UUID-prefixed directory under `~/.ralph/<projectKey>/runs/<runId>/` containing `pipeline.jsonl`, `checkpoint.json`, and per-node raw outputs — a structured agent memory that survives sessions and crashes. But ralph offers **no command to enumerate it**. `pipeline trace <runId>` requires a runId, `pipeline list` only walks `pipelines/*.dot`, and `--resume` lists runs only as a side-effect of failing. The persistent memory is real; the read interface is missing.
...
1. `ls ~/.ralph/$(node -e "...derive key...")/runs/` — user must reproduce `deriveProjectKey()` in their head (`src/cli/commands/pipeline.ts:55`).
...
7. **(Optional, deferred)** Once this lands, evaluate whether the project-key hashing scheme (`<basename>-<6 hex>`) still earns its keep — the only callers will be `enumerateRuns` and `pipeline run`, and a flat `~/.ralph/runs/<runId>/` with `project` recorded in the trace might be simpler. Don't tangle this with the runs-list ship.

...
- `$summary` — verifier's restatement of the proposal.
- `$explanation` — verifier's rubric evidence (relevance, accuracy, project-fit).
- `$explainer_render` — the 4-section before/after the user just approved at the gate. This is the anchor: the design doc must stay consistent with what the user saw and agreed to.
- `$refinements` — cumulative bullet log with per-entry attribution (Round, Topic, Rationale). Authoritative. Every bullet honored unless a later bullet explicitly overrides it. The rationale lines drive judgment calls.
...
2. **Load context.** Read the illumination in full. Re-read `$explainer_render` — the design doc must elaborate on, not contradict, what the user approved. Walk the `$refinements` log bullet-by-bullet; note any override chains so you know the current-state scope.
...
- Design doc must stay apples-to-apples with `$explainer_render` — same anchors, same before/after framing where relevant. If refinements shifted scope, reflect that shift; the explainer-to-design contract is "elaborate, don't contradict".
